...
//            X509CRLEntryObject, CertStatus, RFC3280CertPathUtilities, PKIXPolicyNode
...
_L6:
        ...
        if (distributionpoint.getCRLIssuer() != null)
        {
            ...
            while (i < distributionpoint.length) 
            {
                if (distributionpoint[i].getTagNo() == 4)
                {
                    try
                    {
                        extendedpkixparameters.add(new X500Principal(distributionpoint[i].getName().getDERObject().getEncoded()));
                    }
                    // Misplaced declaration of an exception variable
        ...
        throws AnnotatedException
    {
        ...
        try
        {
            obj1 = new X509CRLObject(new CertificateList((ASN1Sequence)ASN1Sequence.fromByteArray(x509crl.getEncoded())));
        }
        // Misplaced declaration of an exception variable
        catch (Date date)
        {
        ...
        obj1 = (X509CRLEntryObject)((X509CRLObject) (obj1)).getRevokedCertificate(getSerialNumber(obj));
        if (obj1 != null && (getEncodedIssuerPrincipal(obj).equals(((X509CRLEntryObject) (obj1)).getCertificateIssuer()) || getEncodedIssuerPrincipal(obj).equals(getIssuerPrincipal(x509crl))))
        {
            obj = null;
            x509crl = ((X509CRL) (obj));
            if (((X509CRLEntryObject) (obj1)).hasExtensions())
            {
                try
                {
                    x509crl = DEREnumerated.getInstance(getExtensionValue(((X509Extension) (obj1)), X509Extensions.ReasonCode.getId()));
                }
                // Misplaced declaration of an exception variable
            ...
            if (date.getTime() >= ((X509CRLEntryObject) (obj1)).getRevocationDate().getTime() || x509crl == null || x509crl.getValue().intValue() == 0 || x509crl.getValue().intValue() == 1 || x509crl.getValue().intValue() == 2 || x509crl.getValue().intValue() == 8)
            {
                if (x509crl != null)
                {
                    certstatus.setCertStatus(x509crl.getValue().intValue());
                } else
                {
                    certstatus.setCertStatus(0);
                }
                certstatus.setRevocationDate(((X509CRLEntryObject) (obj1)).getRevocationDate());
            }
        }
    }
...
_L1:
        obj = CRLNumber.getInstance(obj).getPositiveValue();
_L4:
        try
        {
...
_L3:
    ...
    protected static DERObject getExtensionValue(X509Extension x509extension, String s)
        throws AnnotatedException
    {
    ...
    private static DERObject getObject(String s, byte abyte0[])
        throws AnnotatedException
    {
        try
        {
        ...
        throws CertPathValidatorException
    {
        ...
        if (asn1sequence != null)
        {
            ...
            while (asn1sequence.hasMoreElements()) 
            {
                try
                {
                    asn1outputstream.writeObject(asn1sequence.nextElement());
...
_L5:
        extendedpkixparameters = DERGeneralizedTime.getInstance(ASN1Object.fromByteArray(extendedpkixparameters));
_L8:

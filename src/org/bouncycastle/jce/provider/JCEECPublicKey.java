    ...
    implements ECPublicKey, org.bouncycastle.jce.interfaces.ECPublicKey, ECPointEncoder
{
    ...
    public JCEECPublicKey(String s, org.bouncycastle.jce.spec.ECPublicKeySpec ecpublickeyspec)
    {
        ...
        if (q.getCurve() == null)
        {
            q = ProviderUtil.getEcImplicitlyCa().getCurve().createPoint(q.getX().toBigInteger(), q.getY().toBigInteger(), false);
        }
        ecSpec = null;
    }
...
label0:
        {
            if (subjectpublickeyinfo.getAlgorithmId().getObjectId().equals(CryptoProObjectIdentifiers.gostR3410_2001))
            {
                ...
                try
                {
                    obj = (ASN1OctetString)ASN1Object.fromByteArray(((DERBitString) (obj)).getBytes());
                }
            ...
            Object obj1 = new X962Parameters((DERObject)subjectpublickeyinfo.getAlgorithmId().getParameters());
            DEROctetString deroctetstring;
            byte abyte3[];
            if (((X962Parameters) (obj1)).isNamedCurve())
            {
                DERObjectIdentifier derobjectidentifier = (DERObjectIdentifier)((X962Parameters) (obj1)).getParameters();
                X9ECParameters x9ecparameters1 = ECUtil.getNamedCurveByOid(derobjectidentifier);
                obj1 = x9ecparameters1.getCurve();
                EllipticCurve ellipticcurve1 = EC5Util.convertCurve(((ECCurve) (obj1)), x9ecparameters1.getSeed());
                ecSpec = new ECNamedCurveSpec(ECUtil.getCurveName(derobjectidentifier), ellipticcurve1, new java.security.spec.ECPoint(x9ecparameters1.getG().getX().toBigInteger(), x9ecparameters1.getG().getY().toBigInteger()), x9ecparameters1.getN(), x9ecparameters1.getH());
            } else
            if (((X962Parameters) (obj1)).isImplicitlyCA())
            {
                ecSpec = null;
                obj1 = ProviderUtil.getEcImplicitlyCa().getCurve();
            } else
            {
                X9ECParameters x9ecparameters = new X9ECParameters((ASN1Sequence)((X962Parameters) (obj1)).getParameters());
            ...
            if ((new X9IntegerConverter()).getByteLength(((ECCurve) (obj1))) >= abyte3.length - 3)
            {
                try
                {
                    subjectpublickeyinfo = (ASN1OctetString)ASN1Object.fromByteArray(abyte3);
                }
                // Misplaced declaration of an exception variable
                catch (SubjectPublicKeyInfo subjectpublickeyinfo)
                {
    ...
    {
        populateFromPubKeyInfo(SubjectPublicKeyInfo.getInstance(ASN1Object.fromByteArray((byte[])(byte[])objectinputstream.readObject())));
...
_L1:
        ...
        obj = new SubjectPublicKeyInfo(new AlgorithmIdentifier(CryptoProObjectIdentifiers.gostR3410_2001, ((ASN1Encodable) (obj)).getDERObject()), new DEROctetString(abyte0));
_L4:
...
_L5:
        ASN1OctetString asn1octetstring = (ASN1OctetString)(new X9ECPoint(engineGetQ().getCurve().createPoint(getQ().getX().toBigInteger(), getQ().getY().toBigInteger(), withCompression))).getDERObject();
        obj = new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_ecPublicKey, ((ASN1Encodable) (obj)).getDERObject()), asn1octetstring.getOctets());

...
_L6:
        ...
        throws IOException
    {
        ...
        if (inputstream != null)
        {
            ...
            byte abyte1[] = new byte[inputstream.readInt()];
            inputstream.readFully(abyte1);
            i = inputstream.readInt();
            byte abyte0[] = new HMac(new SHA1Digest());
            if (ac != null && ac.length != 0)
            {
                ac = PBEParametersGenerator.PKCS12PasswordToBytes(ac);
                PKCS12ParametersGenerator pkcs12parametersgenerator = new PKCS12ParametersGenerator(new SHA1Digest());
                pkcs12parametersgenerator.init(ac, abyte1, i);
                org.bouncycastle.crypto.CipherParameters cipherparameters = pkcs12parametersgenerator.generateDerivedMacParameters(abyte0.getMacSize());
                Arrays.fill(ac, (byte)0);
                abyte0.init(cipherparameters);
                loadStore(new MacInputStream(inputstream, abyte0));
                ac = new byte[abyte0.getMacSize()];
                abyte0.doFinal(ac, 0);
                abyte0 = new byte[abyte0.getMacSize()];
            ...
            {
                loadStore(inputstream);
                inputstream.readFully(new byte[abyte0.getMacSize()]);
                return;
            }
        }
    }
        ...
        throws IOException
    {
        ...
        MacOutputStream macoutputstream = new MacOutputStream(outputstream, hmac);
        PKCS12ParametersGenerator pkcs12parametersgenerator = new PKCS12ParametersGenerator(new SHA1Digest());
        ac = PBEParametersGenerator.PKCS12PasswordToBytes(ac);

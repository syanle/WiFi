    ...
    implements CertSelector, Selector
{
    ...
    {
        form = new V2Form(new GeneralNames(new DERSequence(new GeneralName(x509principal))));
    }
    ...
    private Object[] getNames()
    {
        ...
        while (i != obj.length) 
        {
            if (obj[i].getTagNo() == 4)
            {
                try
                {
                    arraylist.add(new X500Principal(((ASN1Encodable)obj[i].getName()).getEncoded()));
                }
...
_L2:
        ...
        flag = (new X500Principal(((ASN1Encodable)generalname.getName()).getEncoded())).equals(x500principal);
        if (!flag)
        {
            break MISSING_BLOCK_LABEL_70;
        }

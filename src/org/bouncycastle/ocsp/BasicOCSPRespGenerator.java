...
//            OCSPException, RespID, OCSPUtil, BasicOCSPResp, 
    ...
    private class ResponseObject
    {
        ...
        public ResponseObject(CertificateID certificateid, CertificateStatus certificatestatus, Date date, Date date1, X509Extensions x509extensions)
        {
            ...
            } else
            {
                ...
                {
                    certStatus = new CertStatus(new RevokedInfo(new DERGeneralizedTime(getRevocationTime()), new CRLReason(getRevocationReason())));
                } else
                {
                    certStatus = new CertStatus(new RevokedInfo(new DERGeneralizedTime(getRevocationTime()), null));
                }
            }
...
_L1:
        ...
        do
        {
            try
            {
                ...
                s.add(new X509CertificateStructure((ASN1Sequence)ASN1Object.fromByteArray(ax509certificate[i].getEncoded())));
            }
            // Misplaced declaration of an exception variable
            catch (String s)
            {

...
public class WifiAsyncHttpClient
{
...
_L4:
    ...
    protected void sendRequest(DefaultHttpClient defaulthttpclient, HttpContext httpcontext, HttpUriRequest httpurirequest, String s, AsyncHttpResponseHandler asynchttpresponsehandler, Context context)
    {
        if (s != null && context != null)
        {
            ...
            s1 = s.split(";")[0];
            s = s.split(";")[1];
            s1 = Tools.getSign((new StringBuilder("user-agent=")).append(((String) (obj))).append("&action=").append(s).append("&termac=").append(getLocalMacAddress(context)).append("&tertype=android").append("&timestamp=").append(Long.toString(time)).append("&appkey=").append(context.getString(0x7f060001)).append("&secret=").append(context.getString(0x7f060002)).append("&").append(s1).toString());
            httpurirequest.addHeader("user-agent", ((String) (obj)));
            httpurirequest.addHeader("action", s);
            httpurirequest.addHeader("termac", getLocalMacAddress(context));
            httpurirequest.addHeader("tertype", "android");
            httpurirequest.addHeader("timestamp", Long.toString(time));
            httpurirequest.addHeader("appkey", context.getString(0x7f060001));
            httpurirequest.addHeader("sign", s1);
        }

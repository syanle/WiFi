...
abstract class SIGBase extends Record
{
    ...
    private static final long serialVersionUID = 0xcc1e60a20e13656fL;
    protected int alg;
        ...
        throws IOException
    {
        ...
        if (covered < 0)
        {
            throw tokenizer.exception((new StringBuilder("Invalid type: ")).append(s).toString());
        }
        s = tokenizer.getString();
        alg = DNSSEC.Algorithm.value(s);
        if (alg < 0)
        {
            throw tokenizer.exception((new StringBuilder("Invalid algorithm: ")).append(s).toString());
        } else
        {
        ...
        throws IOException
    {
        ...
        expire = new Date(dnsinput.readU32() * 1000L);
        timeSigned = new Date(dnsinput.readU32() * 1000L);

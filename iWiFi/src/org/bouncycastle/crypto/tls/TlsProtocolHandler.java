...
//            LegacyTlsClient, TlsClientContextImpl, TlsInputStream, TlsOutputStream, 
//            CertificateVerifyer
...
_L8:
        ...
        assertEmpty(abyte0);
        if (!Arrays.constantTimeAreEqual(TlsUtils.PRF(securityParameters.masterSecret, "server finished", rs.getCurrentHash(), 12), abyte1))
        {
            failWithError((short)2, (short)40);
        }
        connection_state = 12;
...
_L3:
        ...
        assertEmpty(abyte0);
        boolean flag = hashtable.containsKey(EXT_RenegotiationInfo);
        if (flag && !Arrays.constantTimeAreEqual((byte[])(byte[])hashtable.get(EXT_RenegotiationInfo), createRenegotiationInfo(emptybuf)))
        {
            failWithError((short)2, (short)40);
        }
        tlsClient.notifySecureRenegotiation(flag);
        if (clientExtensions != null)
        {
            tlsClient.processServerExtensions(hashtable);
        }
...
_L7:
        ...
        {
            TlsCredentials tlscredentials = authentication.getClientCredentials(certificateRequest);
            if (tlscredentials == null)
            {
                keyExchange.skipClientCredentials();
                abyte0 = Certificate.EMPTY_CHAIN;
            } else
            {
        ...
        {
            sendCertificateVerify(((TlsSignerCredentials)abyte0).generateCertificateSignature(rs.getCurrentHash()));
            connection_state = 8;
        }
        abyte0 = new byte[1];
        abyte0[0] = 1;
        ...
        connection_state = 9;
        abyte0 = keyExchange.generatePremasterSecret();
        securityParameters.masterSecret = TlsUtils.PRF(abyte0, "master secret", TlsUtils.concat(securityParameters.clientRandom, securityParameters.serverRandom), 48);
        Arrays.fill(abyte0, (byte)0);
        rs.clientCipherSpecDecided(tlsClient.getCompression(), tlsClient.getCipher());
        abyte0 = TlsUtils.PRF(securityParameters.masterSecret, "client finished", rs.getCurrentHash(), 12);
...
_L5:
        connection_state;
        JVM INSTR tableswitch 2 3: default 1032
    //                   2 1045
    //                   3 1059;
           goto _L12 _L13 _L14
_L12:
...
_L6:
        connection_state;
        JVM INSTR tableswitch 3 4: default 1104
    //                   3 1117
    //                   4 1126;
           goto _L16 _L17 _L18
_L16:
...
_L18:
        ...
        assertEmpty(abyte0);
        abyte0 = new Vector();
        for (ByteArrayInputStream bytearrayinputstream1 = new ByteArrayInputStream(abyte3); bytearrayinputstream1.available() > 0; abyte0.addElement(X500Name.getInstance(ASN1Object.fromByteArray(TlsUtils.readOpaque16(bytearrayinputstream1))))) { }
...
_L20:
        ...
        throws IOException
    {
        ...
        if (clientExtensions != null)
        {
            ByteArrayOutputStream bytearrayoutputstream = new ByteArrayOutputStream();
            Integer integer;
            for (Enumeration enumeration = clientExtensions.keys(); enumeration.hasMoreElements(); writeExtension(bytearrayoutputstream, integer, (byte[])(byte[])clientExtensions.get(integer)))
            {

    ...
    implements CertSelector, Selector
{
    ...
    {
        holder = new Holder(new ObjectDigestInfo(i, s1, new AlgorithmIdentifier(s), Arrays.clone(abyte0)));
    }
        ...
        throws CertificateParsingException
    {
        ...
        holder = new Holder(new IssuerSerial(generateGeneralNames(x509principal), new DERInteger(x509certificate.getSerialNumber())));
    }
    ...
    {
        holder = new Holder(new IssuerSerial(new GeneralNames(new DERSequence(new GeneralName(x509principal))), new DERInteger(biginteger)));
    }
    ...
    private Object[] getNames(GeneralName ageneralname[])
    {
        ...
        while (i != ageneralname.length) 
        {
            if (ageneralname[i].getTagNo() == 4)
            {
                try
                {
                    arraylist.add(new X500Principal(((ASN1Encodable)ageneralname[i].getName()).getEncoded()));
                }
                // Misplaced declaration of an exception variable
...
_L2:
        ...
        flag = (new X509Principal(((ASN1Encodable)generalname.getName()).getEncoded())).equals(x509principal);
        if (!flag)
        {
            break MISSING_BLOCK_LABEL_70;
        }

...
_L2:
        ...
        checkCRL(new DistributionPoint(new DistributionPointName(0, new GeneralNames(new GeneralName(4, ((org.bouncycastle.asn1.ASN1Encodable) (obj))))), null, null), (ExtendedPKIXParameters)extendedpkixparameters.clone(), x509certificate, date, x509certificate1, publickey, certstatus, reasonsmask, list);
        flag1 = true;
        obj = extendedpkixparameters1;
_L6:
        if (!flag1)
        ...
        if (obj != null)
        {
            for (int j = 0; j < ((ASN1Sequence) (obj)).size(); j++)
            {
                ...
                try
                {
                    ...
                    derobjectidentifier = DERObjectIdentifier.getInstance(((ASN1Sequence) (obj1)).getObjectAt(0));
        ...
        if (distributionpoint.getCRLIssuer() != null)
        {
            ...
            while (i < distributionpoint.length) 
            {
                ...
                if (distributionpoint[i].getTagNo() == 4)
                {
                    ...
                    try
                    {
                        flag3 = Arrays.areEqual(distributionpoint[i].getName().getDERObject().getEncoded(), abyte0);
                    }
                    // Misplaced declaration of an exception variable
...
_L1:
        ...
        if (distributionpointname.getType() == 1)
        {
            ...
            try
            {
                for (x509crl = ASN1Sequence.getInstance(ASN1Sequence.fromByteArray(CertPathValidatorUtilities.getIssuerPrincipal(x509crl).getEncoded())).getObjects(); x509crl.hasMoreElements(); asn1encodablevector.add((DEREncodable)x509crl.nextElement())) { }
            }
            // Misplaced declaration of an exception variable
            catch (DistributionPoint distributionpoint)
            {
...
_L3:
        ...
        if (distributionpointname1.getType() == 1)
        {
            ...
            do
            {
                ...
                x509crl = ASN1Sequence.getInstance(distributionpoint[j].getName().getDERObject()).getObjects();
                ASN1EncodableVector asn1encodablevector1 = new ASN1EncodableVector();
                for (; x509crl.hasMoreElements(); asn1encodablevector1.add((DEREncodable)x509crl.nextElement())) { }
...
_L2:
        ...
        throws AnnotatedException
    {
        ...
        {
            x509crl = new ReasonsMask(distributionpoint.getReasons().intValue());
        }
        if (issuingdistributionpoint == null)
        {
        ...
        {
            distributionpoint = new ReasonsMask(issuingdistributionpoint.getOnlySomeReasons().intValue());
        }
...
label0:
        {
            ...
            do
            {
                ...
                derobjectidentifier = ((PolicyInformation) (obj3)).getPolicyIdentifier();
                ((Set) (obj1)).add(derobjectidentifier.getId());
                if (!"2.5.29.32.0".equals(derobjectidentifier.getId()))
                {
                    try
                    {
                    ...
                    if (!CertPathValidatorUtilities.processCertD1i(k, alist, derobjectidentifier, ((Set) (obj3))))
                    {
                        CertPathValidatorUtilities.processCertD1ii(k, alist, derobjectidentifier, ((Set) (obj3)));
                    }
                }
            } while (true);

...
_L3:
        if (j != ordering.size())
        {
            if (hashtable.get(vector = (DERObjectIdentifier)ordering.elementAt(j)) == null)
            {
...
_L1:
    ...
    public X509Name(ASN1Sequence asn1sequence)
    {
        ...
        {
            ASN1Set asn1set = ASN1Set.getInstance(((DEREncodable)enumeration.nextElement()).getDERObject());
            int i = 0;
            while (i < asn1set.size()) 
            {
                asn1sequence = ASN1Sequence.getInstance(asn1set.getObjectAt(i));
                if (asn1sequence.size() != 2)
                {
                    throw new IllegalArgumentException("badly sized pair");
                }
                ordering.addElement(DERObjectIdentifier.getInstance(asn1sequence.getObjectAt(0)));
                asn1sequence = asn1sequence.getObjectAt(1);
                if ((asn1sequence instanceof DERString) && !(asn1sequence instanceof DERUniversalString))
                {
                    asn1sequence = ((DERString)asn1sequence).getString();
                    Vector vector;
                ...
                {
                    values.addElement((new StringBuilder()).append("#").append(bytesToString(Hex.encode(asn1sequence.getDERObject().getDEREncoded()))).toString());
                }
                vector = added;
                if (i != 0)
                {
    ...
    private void appendValue(StringBuffer stringbuffer, Hashtable hashtable, DERObjectIdentifier derobjectidentifier, String s)
    {
...
label0:
    ...
    private String canonicalize(String s)
    {
        ...
        if (s.length() > 0 && s.charAt(0) == '#')
        {
            ...
            {
                return Strings.toLowerCase(((DERString)asn1object).getString().trim());
            }
        }
        return s;
    }

    private DERObjectIdentifier decodeOID(String s, Hashtable hashtable)
    {
        if (Strings.toUpperCase(s).startsWith("OID."))
        {
            hashtable = new DERObjectIdentifier(s.substring(4));
        } else
        {
            ...
            DERObjectIdentifier derobjectidentifier = (DERObjectIdentifier)hashtable.get(Strings.toLowerCase(s));
    ...
    private ASN1Object decodeObject(String s)
    {
        try
        {
...
_L6:
        derobjectidentifier = (DERObjectIdentifier)ordering.elementAt(i);
...
_L8:
        if (aflag[k] || !derobjectidentifier.equals((DERObjectIdentifier)((X509Name) (obj)).ordering.elementAt(k)) || !equivalentStrings(s, (String)((X509Name) (obj)).values.elementAt(k))) goto _L12; else goto _L11
_L11:
        aflag[k] = true;
        k = 1;
_L15:
...
_L9:
    ...
    public boolean equals(Object obj, boolean flag)
    {
        ...
        for (int i = 0; i < j; i++)
        {
            if (((DERObjectIdentifier)ordering.elementAt(i)).equals((DERObjectIdentifier)((X509Name) (obj)).ordering.elementAt(i)))
            {
    ...
    public Vector getValues(DERObjectIdentifier derobjectidentifier)
    {
        ...
        while (i != values.size()) 
        {
            if (ordering.elementAt(i).equals(derobjectidentifier))
            {
    ...
    public DERObject toASN1Object()
    {
        if (seq == null)
        {
            ...
            while (i != ordering.size()) 
            {
                ...
                DERObjectIdentifier derobjectidentifier1 = (DERObjectIdentifier)ordering.elementAt(i);
                asn1encodablevector2.add(derobjectidentifier1);
                String s = (String)values.elementAt(i);
                asn1encodablevector2.add(converter.getConvertedValue(derobjectidentifier1, s));
                if (derobjectidentifier == null || ((Boolean)added.elementAt(i)).booleanValue())
                {
    ...
    public String toString(boolean flag, Hashtable hashtable)
    {
        ...
        while (i < ordering.size()) 
        {
            ...
            {
                stringbuffer.append('+');
                appendValue(stringbuffer, hashtable, (DERObjectIdentifier)ordering.elementAt(i), (String)values.elementAt(i));
            } else
            {
